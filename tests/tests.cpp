...
void setup_projection(DP::Projection& proj) {
	constexpr double tfr(7.33), srb(101.4);
	constexpr double pasfrs[DP::N_AGE_BIRTH] = {
	...
	constexpr double pop_t0[DP::N_AGE_BIRTH] = {
	...
	constexpr double pop_t1[DP::N_AGE_BIRTH] = {
	...
	constexpr double prev[DP::N_AGE_BIRTH] = {
	...
	constexpr double frr_age[DP::N_AGE_BIRTH] = {
	...
	constexpr double frr_art[DP::N_AGE_BIRTH] = {
	...
	constexpr double frr_cd4[DP::N_HIV_ADULT] = {1.00, 1.00, 0.96, 0.85, 0.61, 0.38, 0.30};
	constexpr double cd4[DP::N_HIV_ADULT] = {0.0000, 0.5735, 0.2186, 0.1592, 0.0420, 0.0058, 0.0009};
	constexpr double dtx[DP::N_DTX] = {0.40, 0.10, 0.10, 0.05, 0.05, 0.30};
	constexpr int yidx_final(1);

	proj.dat.tfr(yidx_final, tfr);
	proj.dat.srb(yidx_final, srb);
	for (int b = 0; b < DP::N_AGE_BIRTH; ++b) {
		constexpr double frr_loc(0.96);
		constexpr int yidx_first(0);
		const int a = b + DP::AGE_BIRTH_MIN;
		...
		for (int h = DP::HIV_ADULT_MIN; h <= DP::HIV_ADULT_MAX; ++h) {
			for (int d = DP::DTX_MIN; d <= DP::DTX_MAX; ++d) {
...
TEST_CASE("test births", "[births]") {
	constexpr int year_first(1970), year_final(1971), num_years(2);
	constexpr double target_births(251855), tolerance(0.5);
	double births;
	...
	REQUIRE( fabs(births - target_births) < tolerance );
}
...
TEST_CASE("test births HIV exposed", "[births]") {
	...
	REQUIRE( fabs(births_exposed - target_births) < tolerance );
}
